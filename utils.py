...
B = TypeVar("B", models.Person, models.Position, models.PersonResponse, models.Subject)
class_to_file = {
    models.Person: "persons.json",
    models.Position: "positions.json",
    models.Subject: "subjects.json",
}


def override_models_file(data: list[B], cls: type[B]) -> None:
...
def append_persons_to_file(data: list[models.Person]) -> None:
    if not data:
        return
...
def get_person_responses() -> list[models.PersonResponse] | list:
    positions: list[models.Position] = get_data_by_class(models.Position)
    persons: list[models.Person] = get_data_by_class(models.Person)
    data = list()
