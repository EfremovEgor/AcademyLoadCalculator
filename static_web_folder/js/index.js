...
async function setSelectOptions() {
  ...
  const positions = await eel.get_positions()();
...
async function refreshTable() {
  ...
  const persons = await eel.get_persons()();
...
async function submitCreateEmployeeForm() {
  ...
  for (let data_form of forms) {
    ...
    const subject = Array.from(
      await eel.get_subjects_by_field(
  ...
  await eel.save_person(value)();
  await refreshTable();
}
async function submitEditEmployeeForm() {
  ...
  subjects.innerHTML = "";
  await eel.override_person_with_uuid(value)();
  await refreshTable();
...
async function deletePerson() {
  await eel.delete_person_by_uuid(currentEditPerson)();
  closeModals();
  await refreshTable();
}
async function refreshEditForm() {
  ...
  form.reset();
  const data = await eel.get_person_by_uuid(currentEditPerson)();
...
async function addSubjectEditPerson() {
  ...
  const subjects = $("#edit_person_add_subject")[0];
  const subjectObjects = await eel.get_subjects()();
...
async function createFilledSubjectForm(subjectItem) {
  ...
  const subjects = $("#edit_person_add_subject")[0];
  let subjectObjects = await eel.get_subjects()();
  for (let subject of getUniqueArrayByOneField("name", subjectObjects)) {
    const option = document.createElement("option");
  ...
  subjectObjects = await eel.get_subjects_by_field(
    ["name"],
    [subjectItem.name]
  )();
  ...
  subjectObjects = await eel.get_subjects_by_field(
    ["name", "group_name"],
    [subjectItem.name, subjectItem.group_name]
  )();
  ...
  subjectObjects = await eel.get_subjects_by_field(
    ["name", "group_name", "course"],
    [subjectItem.name, subjectItem.group_name, subjectItem.course]
  )();
  ...
  subjectObjects = await eel.get_subjects_by_field(
    ["name", "group_name", "course", "semester"],
    [
      subjectItem.name,
...
async function updateSubjectSelect(event) {
  ...
  switch (element.id) {
    case "name": {
      ...
      const subjects = await eel.get_subjects_by_field(
        ["name"],
        [element.value.trim()]
      )();
    ...
    case "group_name": {
      ...
      const subjects = await eel.get_subjects_by_field(
        ["name", "group_name"],
        [nameSelect.value.trim(), element.value.trim()]
      )();
